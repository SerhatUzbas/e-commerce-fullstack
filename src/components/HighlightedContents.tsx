...
const HighlightedContents = ({ src, text }: HighlightedContentsProps) => {
	...
	return (
		<Box ref={ref} style={{ borderRadius: 6 }}>
			<Box w='100%' h={187} style={{ overflow: 'hidden' }}>
				<Image src={src} height={187} width='100%' style={{ transform: hovered ? 'scale(1.03)' : 'none', transition: '.8s ease' }} />
			</Box>
			<Box h={38} pt={12} py={9} pl={30} bg={hovered ? '#f28232' : '#1b1b1b'} style={{ borderBottomLeftRadius: 6, borderBottomRightRadius: 6, transition: '.5s ease' }}>
